...
- A **wrong link** attaches a homonym's facts, and — measured on a dedicated GPU pass over 1,025 wrong-linked probes — is **approximately neutral** (accuracy 0.311 → 0.325): the model largely ignores evidence irrelevant to the question. Hedged framing ("may be unrelated — ignore if so") changes nothing in either direction.
